...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg):
    ...
    aa = kk.split(".")[-1]
    out_put_file_name = kk.replace(f".{aa}", "Enc'd.and.Upl'd.by.@nflix_movies.mkv")
    #out_put_file_name = video_file + "_compressed" + ".mkv"
    progress = output_directory + "/" + "progress.txt"
    with open(progress, 'w') as f:
      pass
       -metadata title='Encoded and Uploaded by @nflix_movies' -vf drawtext=fontfile=Italic.ttf:fontsize=20:fontcolor=black:x=15:y=15:text='@nflix_movies'
    ##"-metadata", "title=@SenpaiAF", "-vf", "drawtext=fontfile=njnaruto.ttf:fontsize=20:fontcolor=black:x=15:y=15:text=" "Dark Encodes",
     ...
     ## lol 😂
    crf.append("32")
    watermark.append('-vf "drawtext=fontfile=font.ttf:fontsize=25:fontcolor=white:bordercolor=black@0.50:x=w-tw-10:y=10:box=1:boxcolor=black@0.5:boxborderw=6:text=@nflix_movies"')
